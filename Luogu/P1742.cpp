...
Point circumcenter(Point a,Point b,Point c){
    ...
    return (Point){(B2*C1-B1*C2)/(B2*A1-B1*A2),(A2*C1-A1*C2)/(A2*B1-A1*B2)};
}
...
void min_enclosing_circle(){
    q.r=-1e10;
    for(int i=1;i<=n;i++){
        if(in_circle(p[i],q)) continue;
        q.o=p[i];q.r=0;
        for(int j=1;j<=n;j++){
            if(in_circle(p[j],q)) continue;
            q.o=middle_point(p[i],p[j]);
            q.r=distance(p[i],p[j])/2;
            for(int k=1;k<=n;k++){
                if(in_circle(p[k],q)) continue;

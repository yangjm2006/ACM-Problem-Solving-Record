...
void merge(int u, int v)
{
    ...
    for (auto x : st[fa[v]])
    {
        if ((*prev(st[fa[u]].lower_bound(make_pair(x.first, 0)))).second < x.second)
        {
...
void dfs(int u)
{
    ...
    {
        dfs(v);
        if ((*prev(st[find(v)].upper_bound(make_pair(w[u], 0)))).first <= w[u])
        {
            cnt1 += (*prev(st[find(v)].upper_bound(make_pair(w[u], 0)))).second;
        }
